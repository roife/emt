...
If you enable =emt-mode= and the module cannot be found, it will prompt whether to automatically download it from GitHub. Or you can manually retrieve the pre-built module from the [[https://github.com/roife/emt/releases][releases]] section and place =libEMT.dylib= and =libEmacsSwiftModuleDynamic.dylib= in the same directory as =emt-lib-path= (by default, it is located at =modules/libEMT.dylib= within your personal configuration folder, normally =~/.emacs.d/modules/libEMT.dylib=).

*Current version of the dynamic module is v2.1.2, make sure you have updated to latest module.*
